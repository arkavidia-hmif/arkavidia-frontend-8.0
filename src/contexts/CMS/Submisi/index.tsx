...
import { getAllTeams } from '@src/services/admin'
...
function CMSSubmisi() {
  ...
  const [teamList, setTeamList] = useState<TeamCMSProps[] | []>([])
  ...
  const fetchTeamData = async () => {
    const response = await getAllTeams(viewedCategory)
    ...
    <section className="w-full flex flex-col justify-start items-center gap-14">
      <div className="w-full flex justify-center items-center gap-9">
        ...
        <div className="w-1/4">
          <Dropdown
            ...
            isFull={true}
            noSelectedText="competitive-programming"
          />
        </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col w-full gap-10 items-center justify-start">
          {teamList?.map(team => {
            return <TeamCMS {...team} key={`${team.id}${team.team_name}`} />
          })}
        </div>
      )}
    </section>
