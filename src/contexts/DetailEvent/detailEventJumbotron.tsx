...
const EventDetail = (props: any) => {
    ...
    let Title = ''
    let Desc = ''
    const eventTitle = ['ARKAVTALKS', 'ARKAVIDIA FOR INDONESIA', 'ARKAVIDIA GOES TO SCHOOL', 'ACADEMYA']
    const eventDesc = [
    ...
    return (
        <div>
            <div className='mt-[-100px] relative' style={{
                backgroundColor: '#1F307C', borderRadius: '30% 30% 0 0', width: '100%', height: '100%',
            }}>
                <div className='relative'>
                    <div className='font-bold font-archivo flex justify-center mx-[72px] md:mx-[180px] lg:mx-[300px] text-center text-white text-[24px] md:text-[42px] lg:text-[60px] pt-[40px] md:pt-[75px] lg:pt-[130px] pb-[3px] left-text-shadow-3 md:left-text-shadow-4 lg:left-text-shadow-5'>{Title}</div>
                    <div className='font-helvatica lg:font-varela mx-[72px] md:mx-[144px] lg:mx-[360px] mt-[12px] lg:mt-[20px] pb-[50px] md:pb-[80px] lg:pb-[120px] text-justify text-[12px] md:text-[15px] lg:text-[20px] text-white flex justify-center'>{Desc}</div>
                    <div className='flex justify-between invisible lg:visible'>
                        <div className='absolute bottom-0 right-0 z-50 mt-0'><Image src={arkavperson} /></div>
                        <div className='absolute bottom-0 mt-0 z-50 order-first'><Image src={fox} /></div>
                    </div>
                </div>
            </div>
        </div>
...
const DetailEventJumbotron = (props: any) => {
    ...
    let isPublic = true
    eventName === 'AFI' ? (EventLogo = AFILogo) : (eventName === 'Arkavtalks' ? EventLogo = arkavtalkLogo : (eventName === 'AGTS' ? EventLogo = AGTSLogo : EventLogo = academyaLogo))
    ...
    return (
        <div>
            <div className="bg-yellow300 pt-[30px] md:pt-[50px] lg:pt-[100px]"
                style={{
                    backgroundImage: 'linear-gradient(45deg, #DF9800 25%, transparent 25%), linear-gradient(135deg, #DF9800 25%, transparent 25%),linear-gradient(45deg, transparent 75%, #DF9800 75%),linear-gradient(135deg, transparent 75%, #DF9800 75%)',
                    backgroundSize: '180px 180px',
                    backgroundPosition: '0 0, 90px 0, 90px -90px, 0px 90px'
                }}>
                <div className='relative'>
                    <div className='relative flex justify-center z-50 mt-[15px] lg:pt-[15px] scale-[0.5] md:scale-[0.7] lg:scale-[1]'>
                        <Image src={EventLogo} />
                        <div className='absolute z-10 -scale-x-[0.8] scale-y-[0.8] md:-scale-x-[0.9] md:scale-y-[0.9] lg:-scale-x-[1.0] lg:scale-y-[1.0] top-[120px] left-[-200px] md:top-[100px] md:left-[-50px] lg:top-[110px] lg:left-[150px]'><Image src={stars}/></div>
                    </div>
                    <div className=''>
                ...
                {isPublic ? (
                    <>
                        <div className='bg-white text-[24px] truncate text-clip overflow-hidden font-archivo font-bold whitespace-pre py-[8px] border-y-[2px] border-solid border-black'>CHOOSE YOUR PATH!    CHOOSE YOUR PATH!    CHOOSE YOUR PATH!    CHOOSE YOUR PATH!    CHOOSE YOUR PATH!    CHOOSE YOUR PATH!    </div>
                    </>
                ) : (
                    <>
                        <div className='bg-red300 flex justify-center gap-2 md:gap-6 lg:gap-10 items-center'>
                            <div className= 'flex w-[20px] h-[20px] lg:w-[55px] md:w-[35px] md:h-[35px] lg:h-[55px] lg:pt-[5px]'><Image src={warning} /></div>
                            <div className= 'flex text-yellow200 font-archivo font-bold text-[24px] md:text-[42px] lg:text-[60px] gap-2 lg:gap-5 pt-[3px] blue200-text-shadow-2 md:blue200-text-shadow-3 lg:blue200-text-shadow-5'>NOT FOR PUBLIC</div>
                        </div>
                    </>
                )}
            </div>
