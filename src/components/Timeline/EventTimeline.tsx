...
export default function Timeline({ dates }: AppProps) {
  return dates?.length >= 5 ? (
    ...
    <div className="flex justify-center items-center w-[83vw] overflow-x-auto scrollbar scrollbar-thumb-white scrollbar-track-yellow200 pt-16 pb-20 bg-gradient-to-b from-[rgba(217,217,217,0)] to-[#FF634B]">
      {dates.map(({ title, startDate, endDate }, idx) => (
        <_DateComponent
          key={idx}
          title={title}
