...
const PrintCountdown = (d1: number, d2: number, h1: number, h2: number, m1: number, m2: number, s1: number, s2: number) => {
...
const Countdown = ({date, dateString, title} : CountdownProps) => {
    ...
    return (
        <>
            {/* Box */}
            <div className="flex flex-col items-center space-y-2 bg-black pt-1 justify-center overflow-hidden h-[250px]">
                {/* Bagian title */}
                <div className="w-40 font-archivo text-black text-center text-sm lg:bg-black lg:rounded-md lg:text-white">{title}</div>
                {/* Bagian tanggal dalam string */}
                <div className="font-varela text-center text-sm bg-cream400 border-black border-2 rounded-md p-0.5 lg:rounded-none lg:border-none lg:bg-transparent lg:p-0">{dateString}</div>
                {/* Bagian countdown */}
                {habis ? PrintCountdown(0,0,0,0,0,0,0,0) : PrintCountdown(d1, d2, h1, h2, m1, m2, s1, s2)}
            </div>
        </>
  );
};
